        ...
        implements Runnable
{
     ...
     */
    private static SampleLabelJHelpImage sampleLabelJHelpImage;
    ...
    public void run()
    {
        ...
        for (int i = 1; i <= max; i++)
        {
            SampleCanvas1.canvas.getLayer(0)
                                .setZoom((zoom * i) / max);
            SampleCanvas1.canvas.updateImage();
        }

        time = System.nanoTime() - time;
        Debug.println(DebugLevel.INFORMATION, "time=", time / 1000000, "ms : ~", (time / max) / 1000000, "ms per draw");
    }
}