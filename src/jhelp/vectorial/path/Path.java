      ...
      implements VectorialConstants, PathElementChangeListener
{
   ...
   {
      return ((character >= '0') && (character <= '9')) || (character == '.') || (character == '-') || (character == '+') || (character == 'e')
            || (character == 'E');
   }
    ...
    * 
    * @param index
    *           Offset where start reading characters array
    * @param characters
    *           Characters array
    * @param length
    ...
    * @return Created path
    */
   public static Path createArc(final double x, final double y, final double width, final double height, final double angleStart, final double angleTotal,
         final ArcMode arcMode)
   {
      return Path.createCenterArc(x - (width / 2), y - (height / 2), width / 2, height / 2, angleStart, angleStart + angleTotal, arcMode);
   }
    ...
    * @return Created path
    */
   public static Path createCenterArc(final double centerX, final double centerY, final double radiusX, final double radiusY, double angleStart,
         double angleEnd, final ArcMode arcMode)
   {
      angleStart = UtilMath.modulo(angleStart, UtilMath.TWO_PI);
      angleEnd = UtilMath.modulo(angleEnd, UtilMath.TWO_PI);
      ...
      boolean large = UtilMath.modulo(Math.abs(angleEnd - angleStart), UtilMath.TWO_PI) > Math.PI;
    ...
    * @return Created path
    */
   public static Path createCenterEllipse(final double centerX, final double centerY, final double radiusX, final double radiusY)
   {
    ...
    * @return Created path
    */
   public static Path createCenterRectangle(final double centerX, final double centerY, final double width, final double height)
   {
    ...
    * 
    * @param close
    *           Indicates if have close the polyline (to become a polygon)
    * @param points
    *           Points list
    * @return Created path
    */
    ...
    * @return Created path
    */
   public static Path createRoundRectangle(final double x, final double y, final double width, final double height, double arcWidth, double arcHeight)
   {
    ...
    *           Final point Y
    */
   public void cubicBezierTo(final boolean relative, final double firstControlPointX, final double firstControlPointY, final double secondControlPointX,
         final double secondControlPointY, final double x, final double y)
   {
      this.addElement(new CubicBezierTo(relative, firstControlPointX, firstControlPointY, secondControlPointX, secondControlPointY, x, y));
   }
    ...
    * 
    * @param pathDrawer
    *           Path drawer where draw the path
    */
    ...
    * For understand the role of <b>largeArc</b> and <b>sweep</b> launch the sample : {@link MainLargeArcSweepExemple}
    * 
    * @param relative
    *           Indicates if coordinates are relative to current point ({@code true}) OR coordinates are absolute ({@code false}
    *           )
    ...
    * @param largeArc
    *           Indicates if use the large arc
    * @param sweep
    ...
    *           Final point Y
    */
   public void ellipticalArcTo(final boolean relative, final double radiusX, final double radiusY, final double rotationAxisXInDegree, final boolean largeArc,
      ...
      while(index < length)
      {
         ...
         do
         {
            ...
            switch(character)
            {
               ...
               case 'C':
               case 'c':
                  this.addElement(new CubicBezierTo(relative, values[0], values[1], values[2], values[3], values[4], values[5]));
                  referenceX = values[4];
                  referenceY = values[5];
               ...
               case 'S':
               case 's':
                  this.addElement(new CubicBezierTo(relative, referenceX + cubicSecondX, referenceY + cubicSecondY, values[0], values[1], values[2], values[3]));
                  referenceX = values[2];
                  referenceY = values[3];
               ...
               case 'T':
               case 't':
                  this.addElement(new QuadricBezierTo(relative, referenceX + quadricX, referenceY + quadricY, values[0], values[1]));
                  referenceX = values[0];
                  referenceY = values[1];
                  cubicSecondX = 0;
                  cubicSecondY = 0;
               ...
               case 'A':
               case 'a':
                  this.addElement(new EllipticalArcTo(relative, values[0], values[1], values[2], UtilMath.equals(values[3], 1), UtilMath.equals(values[4], 1),
                        values[5], values[6]));
                  referenceX = values[5];
                  referenceY = values[6];
                  cubicSecondX = 0;
    ...
    *           Final point Y
    */
   public void quadricBezierTo(final boolean relative, final double controlPointX, final double controlPointY, final double x, final double y)
   {
   ...
   public void registerPathChangeListener(final PathChangeListener pathChangeListener)
   {
      ...
      {
         throw new NullPointerException("pathChangeListener musn't be null");
      }
