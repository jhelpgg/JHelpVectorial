        ...
        extends PathElement
{
     ...
     * For understand the role of <b>largeArc</b> and <b>sweep</b> launch the sample : MainLargeArcSweepExemple
     *
     * @param relative              Indicates if coordinates are relative to current point ({@code true}) OR coordinates
     *                              are absolute
     *                              ({@code false}
     *                              )
     ...
     */
    public EllipticalArcTo(final boolean relative, final double radiusX, final double radiusY,
     ...
     *
     * @param relative Indicates if coordinates are relative to current point ({@code true}) OR coordinates are absolute
     *                 ({@code false}
     *                 )
            ...
            throws ParsePathException
    {
        ...
        try
        {
            this.largeArc = json.get(VectorialConstants.PARAMETER_LARGE_ARC)
                                .getBoolean();
            this.radiusX = json.get(VectorialConstants.PARAMETER_RADIUS_X)
                               .getNumber();
            this.radiusY = json.get(VectorialConstants.PARAMETER_RADIUS_Y)
                               .getNumber();
            this.rotationAxisXInDegree = json.get(VectorialConstants.PARAMETER_ROTATION_AXIS_X_IN_DEGREE)
                                             .getNumber();
            this.sweep = json.get(VectorialConstants.PARAMETER_SWEEP)
                             .getBoolean();
            this.x = json.get(VectorialConstants.PARAMETER_X)
                         .getNumber();
            this.y = json.get(VectorialConstants.PARAMETER_Y)
                         .getNumber();
        }
    ...
    protected boolean equalsIntern(final PathElement pathElement)
    {
        ...
        return (UtilMath.equals(this.radiusX, ellipticalArcTo.radiusX))//
                && (UtilMath.equals(this.radiusY, ellipticalArcTo.radiusY))//
                && (UtilMath.equals(this.rotationAxisXInDegree, ellipticalArcTo.rotationAxisXInDegree))//
                && (this.largeArc == ellipticalArcTo.largeArc)//
                && (this.sweep == ellipticalArcTo.sweep)//
                && (UtilMath.equals(this.x, ellipticalArcTo.x))//
                && (UtilMath.equals(this.y, ellipticalArcTo.y))//
                ;
    }
    ...
    protected void fillJSON(final ObjectJSON json)
    {
        ...
        json.put(VectorialConstants.PARAMETER_ROTATION_AXIS_X_IN_DEGREE, ValueJSON.newValue(this.rotationAxisXInDegree));
    ...
    {
        if (UtilMath.equals(this.radiusX, radiusX))
        {
            return;
        }
    ...
    {
        if (UtilMath.equals(this.radiusY, radiusY))
        {
            return;
        }
    ...
    {
        if (UtilMath.equals(this.rotationAxisXInDegree, rotationAxisXInDegree))
        {
            return;
        }
