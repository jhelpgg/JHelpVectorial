      ...
      implements ShapeDrawer
{
    ...
    */
   public NeonShapeDrawer(final int width, final int height, final int[] pixels, final int color, final int strokeWidth, final double neonFactor)
   {
      this.width = width;
      this.height = height;
      this.pixels = pixels;
   ...
   public void drawShapeSegment(final Shape shape, final double x1, final double y1, final double x2, final double y2)
   {
      ...
      final int sx = UtilMath.sign(xx - x);
      final int dy = Math.abs(yy - y);
      final int sy = UtilMath.sign(yy - y);
