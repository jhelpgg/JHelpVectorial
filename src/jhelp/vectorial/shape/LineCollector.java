      ...
      implements PathDrawer
{
    ...
    *           Precision to use
    */
   public LineCollector(final List<Line> list, final double x, final double y, final double scaleX, final double scaleY, final int precision)
   {
      if(list == null)
      {
    ...
    *           Y translation applied to path
    * @param precision
    *           Precision to use for approximate curves
    */
   ...
   private void addLine(final double x1, final double y1, final double x2, final double y2)
   {
      ...
      this.left = UtilMath.min(this.left, xx1, xx2);
      this.top = UtilMath.min(this.top, yy1, yy2);
      this.right = UtilMath.max(this.right, xx1, xx2);
      this.bottom = UtilMath.max(this.bottom, yy1, yy2);
    ...
    */
   @Override
   public void cubicBezierTo(final double firstControlPointX, final double firstControlPointY, final double secondControlPointX,
         final double secondControlPointY, final double x, final double y)
   {
      this.bufferX = UtilMath.PCubiques(this.lastX, firstControlPointX, secondControlPointX, x, this.precision, this.bufferX);
      this.bufferY = UtilMath.PCubiques(this.lastY, firstControlPointY, secondControlPointY, y, this.precision, this.bufferY);
    ...
    *           Rotation angle to apply to ellipse in degree
    * @param largeArc
    *           Indicates if large arc implied
    * @param sweep
    *           Indicates if sweep is choose
    * @param x
    *           Final point X
    * @param y
    *           Final point Y
    ...
    */
   @Override
   public void ellipticalArcTo(final double radiusX, final double radiusY, final double rotationAxisXInDegree, final boolean largeArc, final boolean sweep,
         final double x, final double y)
   {
      if((UtilMath.sign(radiusX) <= 0) || (UtilMath.sign(radiusY) <= 0))
      {
         this.lineTo(x, y);
         return;
      }

      if((UtilMath.equals(this.lastX, x) == true) && (UtilMath.equals(this.lastY, y) == true))
      {
         return;
      }

      final EllipticArcToCrawler ellipticArcToCrawler = new EllipticArcToCrawler(this.lastX, this.lastY, radiusX, radiusY, rotationAxisXInDegree, largeArc,
   ...
   {
      this.bufferX = UtilMath.PQuadriques(this.lastX, controlPointX, x, this.precision, this.bufferX);
      this.bufferY = UtilMath.PQuadriques(this.lastY, controlPointY, y, this.precision, this.bufferY);
